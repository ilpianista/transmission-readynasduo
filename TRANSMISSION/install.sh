...
if grep -q ${name} $ADDON_HOME/addons.conf; then
  orig_vers=`awk -F'!!' '/TRANSMISSION/ { print $3 }' $ADDON_HOME/addons.conf | cut -f1 -d'.'`
fi
...
[ -d /c/addons-config/Transmission ] || mkdir -p /c/addons-config/Transmission
chown -R nobody.nogroup /c/addons-config

grep -v TRANSMISSION_RPC_PORT /etc/default/services > /tmp/services
echo "TRANSMISSION_RPC_PORT=9091" >> /tmp/services
